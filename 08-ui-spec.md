...
- Hero. Headline: "Grant compliance built for Alaska." Subhead: "AuroraGrants helps Alaska nonprofits and tribal organizations draft, review, and submit grant reports in a fraction of the time, with human review and tribal data sovereignty built in."
- CTA: "Join the waitlist" and "Read our mission."
...
- Tiers. Table with Free, Solo, Team, Institutional, Sovereignty.
...
- Waitlist form. Email + org type + interest (free, solo, team, institutional, sovereignty).
- Footer. Contact, privacy, terms, mission.
...
- Reports
- Documents
- Settings (org info, users, billing, data sovereignty, audit log, export)

Top bar: org switcher (Clerk), user menu, help.

Persistent "Tier: {tier}" badge in sidebar footer.
...
Current tier. Change tier. Stripe Billing Portal link. Usage stats (drafts this month, tokens used, cost).

## Settings — Data Sovereignty (`/app/settings/sovereignty`)

Only visible on Institutional and Sovereignty tiers.

- LLM provider (Anthropic default / Anthropic ZDR / OpenAI / Google).
- Data residency region.
- ZDR status (green badge if active).
- BYOK key status (Sovereignty only).
- TDUA signed status (Sovereignty only).
- Advisory council contact (Sovereignty only).
- Download signed DPA (Institutional and Sovereignty).
...
Stepper, 6 steps (10 for Sovereignty). Each step saves progress to `tenant_onboarding_state` so user can resume.

## Admin (`/admin`)

Founder-only, gated by email allowlist.
