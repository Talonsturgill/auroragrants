...
### Scope
Next.js 15 App Router scaffold. Supabase project with RLS. Clerk auth with orgs. Stripe test mode. Marketing site shell at `/`. Authenticated app shell at `/app`. CI pipeline (GitHub Actions).
...
6. Stripe test mode. Create products: Solo ($39/mo), Team ($199/mo), Institutional ($9,000/yr), Sovereignty ($18,000/yr). Create a Stripe Checkout endpoint at `/api/billing/checkout` and a webhook handler at `/api/webhooks/stripe`.
7. Marketing site `/` with sections: hero, problem, who-it-is-for, tiers, sovereignty posture, founder note, waitlist form.
8. Authenticated app shell at `/app` with sidebar navigation: Dashboard, Deadlines, Opportunities, Awards, Reports, Documents, Settings.
9. GitHub Actions: lint, typecheck, unit tests, Playwright, deploy to Vercel preview on PR, deploy to Vercel prod on main.
10. Sentry and Axiom (or similar) wired for web and workers.
11. Environment variable matrix documented in `.env.example`.
...
- Stripe Checkout succeeds in test mode and the webhook correctly updates `tenants.tier`.
...
- [ ] Stripe webhook updates tenant tier.
...
### Scope
Deadline agent cron job. First design-partner onboarding wizard. Weekly Deadline Radar email for free tier.
...
2. Deadline Radar weekly email for free tier: for each free tenant, send a Monday 9am Alaska Time digest of the next 10 deadlines for funders they've subscribed to.
...
4. Onboarding wizard at `/app/onboarding`:
   ...
   - Step 6: Tier selection and Stripe Checkout (or "start free" for Deadline Radar).
5. Sovereignty-tier additional steps:
...
## Phase 6 — Tribal Sovereignty tier readiness
...
- Insurance: E&O, cyber, GL bound.
- Foraker Group partnership application submitted.
- 10 warm-intro emails sent.
- Alaska LLC formed, business license, municipality of Anchorage license.
- First paying tenant live on Solo or Team tier.
- At least one design partner under Team or Institutional contract.
