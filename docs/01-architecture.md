...
### External APIs
- Anthropic Messages API — Sonnet 4.5 by default. Opus 4.5 for high-stakes Critic. Haiku 4.5 for classification. ZDR routing required on Sovereignty deployments.
...
**No billing integrations.** AuroraGrants is free. If you need to bill for a derivative service around the product, that is your fork's concern, not upstream's.
...
- Per-tenant `llm_provider` enum: `anthropic` (default), `anthropic_zdr` (Sovereignty deployments), `openai`, `google`.
- Per-tenant `llm_model_override` map: `{ writer: "claude-sonnet-4-5", critic: "claude-opus-4-5", classifier: "claude-haiku-4-5" }`.
- Sovereignty deployments force `anthropic_zdr`. No override.
...
- Database backups encrypted at rest (Supabase default) and weekly off-site to Fly.io volume for Sovereignty deployments.
...
- A `/admin` route restricted to a maintainer email allowlist shows per-tenant token spend, eval drift, and deadline queue depth.
