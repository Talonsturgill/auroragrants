...
describe("PATCH /api/onboarding/org-info", () => {
  it("returns 400 when name is missing", async () => {
    ...
    const res = await PATCH(makeRequest({ ein: "12-3456789" }) as never);
  ...
  it("returns 403 when no tenant is resolved", async () => {
    mockTenantId = null;
    const res = await PATCH(makeRequest({ name: "My Org" }) as never);
  ...
  it("returns 400 for invalid org_type enum value", async () => {
    const res = await PATCH(makeRequest({ name: "Valid Name", org_type: "invalid_type" }) as never);
    expect(res.status).toBe(400);
    const json = await res.json();
    expect(json.error).toBe("invalid_body");
  });
  ...
  it("succeeds and returns the updated tenant when all fields are valid", async () => {
    ...
    const res = await PATCH(
      makeRequest({ name: "Updated Org", ein: "12-3456789", org_type: "501c3" }) as never,
    );
